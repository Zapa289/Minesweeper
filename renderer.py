...
class Renderer:
    ...
    def create_field(self, minefield: ndarray, flag_count: int, time: int) -> None:
        ...
        for row_index, row in enumerate(minefield):
            ...
            for col_index, proximity in enumerate(row):
                tile = Tile(
                    ...
                    proximity=proximity,
                    surface=pygame.transform.scale(self.textures.hidden, TILE_SIZE),
                )
                self.tiles.add(tile)
                x_pos += tile_x_size
            y_pos += tile_y_size
        ...
        self.draw_tray(flag_count, time)
    ...
    def update_timer(self, time: int) -> None:
        ...
        timer_rect = timer_surface.get_rect(
            right=screen_x - EDGE_GAP, centery=TIMER_POS_Y
        )
    ...
    def update_flag_count(self, flag_count: int) -> None:
        ...
        flag_counter_rect = flag_counter_surface.get_rect(
            left=FLAG_POS_X, centery=FLAG_POS_Y
        )
